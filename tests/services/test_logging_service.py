...
from investment_dashboard import config
from investment_dashboard.repositories import app_config_repo
from investment_dashboard.services import logging_service


@pytest.fixture
def default_info_settings(monkeypatch: pytest.MonkeyPatch) -> None:
    """Pin the unconfigured default to INFO.

    ``get_log_level`` falls back to ``Settings.log_level``; other tests in the
    suite can leak ``INV_DASHBOARD_LOG_LEVEL`` into the environment (and clear
    the ``get_settings`` cache), so we stub the lookup here to keep the
    "nothing chosen yet" default deterministic regardless of test order.
    """
    monkeypatch.setattr(config, "get_settings", lambda: SimpleNamespace(log_level="INFO"))


def test_default_level_is_info(session: Session, default_info_settings: None) -> None:
...
def test_corrupt_stored_value_falls_back_to_default(
    session: Session, default_info_settings: None
) -> None:
