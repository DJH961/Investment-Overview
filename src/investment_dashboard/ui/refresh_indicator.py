...
def install_header_indicator(tz: tzinfo | None = None) -> None:
    ...
    with ui.row().classes("items-center no-wrap inv-refresh-chip") as chip:
        icon = ui.icon("sync").classes("inv-refresh-icon")
        label = ui.label("Live").classes("inv-refresh-label")
    tip = ui.tooltip("Automatic price updates are on")
    ...
    def _poll() -> None:
        ...
        if snap.last_update_at is not None:
            ...
            tip.set_text(f"Automatic price updates on \u00b7 last update {when:%H:%M:%S}")
        else:
            tip.set_text("Automatic price updates are on")
