/**
 * The collapsed-session "1W" x-axis: dead time between sessions (nights,
 * weekends, holidays) is dropped by packing each trading day into an equal-width
 * band, the line breaks per session, separators sit in the inter-band gutters,
 * and each band is labelled by weekday + day-of-month. See
 * docs/weekly_chart_session_axis_plan.md.
 */
import { describe, expect, it } from "vitest";

import {
  SESSION_GUTTER,
  sessionAreaPath,
  sessionFractions,
  sessionLinePath,
} from "../src/chart";
import { Decimal } from "../src/decimal-config";

/** Build a run of `count` ISO instants `stepMin` minutes apart from `start`. */
function instants(start: string, count: number, stepMin: number): string[] {
  const base = new Date(start).getTime();
  const out: string[] = [];
  for (let i = 0; i < count; i += 1) out.push(new Date(base + i * stepMin * 60_000).toISOString());
  return out;
}

describe("sessionFractions", () => {
  it("packs each calendar day into an equal-width band, dropping the dead time", () => {
    // Two sessions a full calendar day apart, each ~6.5h of intraday points. On a
    // wall-clock axis the overnight gap would eat most of the width; collapsed,
    // the two bands are equal and adjacent (minus the gutter).
    const dayA = instants("2026-06-22T13:30:00Z", 7, 65); // ~6.5h
    const dayB = instants("2026-06-23T13:30:00Z", 7, 65);
    const layout = sessionFractions([...dayA, ...dayB])!;
    expect(layout).not.toBeNull();
    expect(layout.bands.map((b) => b.day)).toEqual(["2026-06-22", "2026-06-23"]);

    const bandWidth = (1 - SESSION_GUTTER) / 2;
    // Band A spans [0, bandWidth]; band B spans [bandWidth+gutter, 1].
    expect(layout.fractions[0]).toBeCloseTo(0, 6);
    expect(layout.fractions[6]).toBeCloseTo(bandWidth, 6);
    expect(layout.fractions[7]).toBeCloseTo(bandWidth + SESSION_GUTTER, 6);
    expect(layout.fractions[13]).toBeCloseTo(1, 6);
  });

  it("returns each session's first-point index as a line/area boundary", () => {
    const dayA = instants("2026-06-22T13:30:00Z", 3, 120);
    const dayB = instants("2026-06-23T13:30:00Z", 4, 100);
    const dayC = instants("2026-06-24T13:30:00Z", 2, 200);
    const layout = sessionFractions([...dayA, ...dayB, ...dayC])!;
    // First point of each band: 0, 3, 7.
    expect(layout.boundaryIndexes).toEqual([0, 3, 7]);
  });

  it("places one separator per inter-band gutter, at the gutter centre", () => {
    const dayA = instants("2026-06-22T13:30:00Z", 3, 120);
    const dayB = instants("2026-06-23T13:30:00Z", 3, 120);
    const dayC = instants("2026-06-24T13:30:00Z", 3, 120);
    const layout = sessionFractions([...dayA, ...dayB, ...dayC])!;
    expect(layout.separators).toHaveLength(2);
    const bandWidth = (1 - 2 * SESSION_GUTTER) / 3;
    expect(layout.separators[0]).toBeCloseTo(bandWidth + SESSION_GUTTER / 2, 6);
    expect(layout.separators[1]).toBeCloseTo(2 * bandWidth + 1.5 * SESSION_GUTTER, 6);
    // Separators are strictly inside the plot and ascending.
    for (const s of layout.separators) {
      expect(s).toBeGreaterThan(0);
      expect(s).toBeLessThan(1);
    }
  });

  it("places points within a band by their own session's elapsed fraction", () => {
    // A day whose points are NOT evenly spaced: a dense cluster late in the day
    // must sit late in the band, not be spread across it by point count.
    const dates = [
      "2026-06-22T13:30:00Z",
      "2026-06-22T19:00:00Z",
      "2026-06-22T19:30:00Z",
      "2026-06-22T20:00:00Z",
    ];
    const layout = sessionFractions(dates)!;
    expect(layout.bands).toHaveLength(1);
    // Single band fills the full width (no gutter). Open at 0, close at 1.
    expect(layout.fractions[0]).toBeCloseTo(0, 6);
    expect(layout.fractions[3]).toBeCloseTo(1, 6);
    // The 19:00 point is ~85% through the 13:30→20:00 span, so it sits far right.
    expect(layout.fractions[1]).toBeGreaterThan(0.8);
  });

  it("centres a lone close-only day in its band", () => {
    const layout = sessionFractions([
      "2026-06-22T00:00:00Z", // coarse close A
      "2026-06-23T00:00:00Z", // coarse close B
    ])!;
    const bandWidth = (1 - SESSION_GUTTER) / 2;
    expect(layout.fractions[0]).toBeCloseTo(bandWidth / 2, 6);
    expect(layout.fractions[1]).toBeCloseTo(bandWidth + SESSION_GUTTER + bandWidth / 2, 6);
  });

  it("returns null for too few points or non-date labels", () => {
    expect(sessionFractions([])).toBeNull();
    expect(sessionFractions(["2026-06-22T00:00:00Z"])).toBeNull();
    expect(sessionFractions(["not-a-date", "also-not"])).toBeNull();
  });
});

describe("sessionLinePath / sessionAreaPath (break per session)", () => {
  const x = (i: number): number => i;
  const y = (v: number): number => 100 - v;
  const baseline = 100;
  const d = (v: number | null): Decimal | null => (v === null ? null : new Decimal(v));

  it("starts a fresh subpath at every session boundary, never crossing the gap", () => {
    // Three sessions of two points each; boundaries at 0, 2, 4.
    const values = [d(10), d(11), d(12), d(13), d(14), d(15)];
    const boundaries = new Set([0, 2, 4]);
    const line = sessionLinePath(values, x, y, boundaries);
    // One "M" (move = new island) per session — no interpolation Fri→Mon.
    expect((line.match(/M/g) ?? []).length).toBe(3);
    expect((line.match(/L/g) ?? []).length).toBe(3);
  });

  it("also breaks on a null gap within the data", () => {
    const values = [d(10), null, d(12), d(13)];
    const line = sessionLinePath(values, x, y, new Set([0]));
    expect((line.match(/M/g) ?? []).length).toBe(2);
  });

  it("closes one area island per session down to the baseline", () => {
    const values = [d(10), d(11), d(12), d(13)];
    const area = sessionAreaPath(values, x, y, baseline, new Set([0, 2]));
    // Two closed islands (one "Z" each), each touching the baseline twice.
    expect((area.match(/Z/g) ?? []).length).toBe(2);
    expect((area.match(/100\.0/g) ?? []).length).toBe(4);
  });

  it("omits a degenerate single-point session from the area fill", () => {
    // A lone close-only session contributes no two-point island, so no fill.
    const values = [d(10)];
    expect(sessionAreaPath(values, x, y, baseline, new Set([0]))).toBe("");
  });
});
